...
export default {
  ...
  // 获取上周的排行榜数据
  showLastWeek() {
    this.$router.push('/earn_lastweek')
  },
  // 获取排行榜
  getVSoulRank() {
  ...
  // 获取赚取的金额
  getEarnMoney() {
    let url = this.$api.infor.getEarnedMoney
    get(url)
      .then(res => {
        if (res.code === 200) {
          this.earnedMoney = res.data
        }
      })
      .catch(error => {})
  },
  ...
  async BalanceOf() {
    ...
    try {
      ...
      this.PoolBalance = ethers.utils.formatEther(formatNumber(parseInt(BalanceOf._hex)))
    } catch (error) {
      this.PoolBalance = '0'
      // this.$toast(error)
    }
  ...
  getMintedNFTPage(type) {
    ...
    let url = type == 1 ? this.$api.infor.getCollectNFTPage : this.$api.infor.getPicksNFTPage
    this.NftList = []
    get(url, data)
      .then(res => {
        if (res.code === 200) {

...
import { addChain_Params,nftAddress,marketAddress,ABIAddress,getHeight } from "@/libs/common.js";
...
export default {
    ...
    PicklinkUser(id) {
        if (this.pickIsUser(id)) {
            ...
            this.$router.push(
                `/user?id=${this.NFTPickInfo[`indexUserId${id}`]}`
            );
        }
    },
    turnShowClick() {
                ...
                params: [
                    {
                        ...
                        to: marketAddress,
                        value: ethers.utils.parseUnits(this.NFTDetail.price, 18)
                            ._hex,
                    },
                ],
            })
    ...
    getNFTPickInfo() {
        this.overlayshow = true;
        let url =
            this.$api.nft.getNFTPickInfo + `?tokenId=${this.$route.query.id}`;
        get(url)
            .then((res) => {
                if (res.code === 200) {
